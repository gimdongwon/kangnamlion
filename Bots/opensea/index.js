...
function response(room, msg, sender, isGroupChat, replier, ImageDB, packageName) {
  ...
  if (msg.startsWith('옾')) {
    try{
      ...
      */
      let result = opensea_func(symbol);
      let output_text = ''
      output_text += 'nft opensea 조회 \n';
      output_text += '이름 : ' + symbol + '\n';
      output_text +=
        '바닥가 : ' + result.collection.stats['floor_price'].toFixed(2) + ' ' +result.collection.payment_tokens[0]['symbol'] + '\n';
  ...
  /* if (msg.startsWith('사전')){
    ...
    let engName = temp2[1], korName = temp2[2];
