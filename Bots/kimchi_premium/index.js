function response(room, msg, sender, isGroupChat, replier, ImageDB, packageName, threadId) {
  if (msg === '김프') {
    ...
    const upbit = JSON.parse(org.jsoup.Jsoup.connect(url).ignoreContentType(true).get().text());
    ...
    const bith = JSON.parse(org.jsoup.Jsoup.connect(url).ignoreContentType(true).get().text());
    ...
    const binance = JSON.parse(org.jsoup.Jsoup.connect(url).ignoreContentType(true).get().text());
